...
require 'db.php';
$id = $_GET['id'];
$sql = 'SELECT * FROM telecoms WHERE id=:id';
...
if (isset($_POST['nom_offre'])  && isset($_POST['debit']) && isset ($_FILES["image"]) && $_FILES["image"]["error"] == 0)  {
  ...
  if ($file_type != "jpg" && $file_type != "jpeg" && $file_type != "png" && $file_type != "gif") {
    ...
    $sql = 'UPDATE telecoms SET nom_offre=:nom_offre, debit=:debit,prix=:prix,contact=:contact,name=:name ,size=:size ,data=:data ,type=:type  WHERE id=:id';
    $statement = $conn->prepare($sql);
