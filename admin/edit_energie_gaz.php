...
require 'db.php';
$id = $_GET['id'];
$sql = 'SELECT * FROM energie_gaz WHERE id=:id';
...
if (isset($_POST['nom'])  && isset($_POST['prix_an']) && isset($_FILES["image"]) && $_FILES["image"]["error"] == 0) {
  ...
  if ($file_type != "jpg" && $file_type != "jpeg" && $file_type != "png" && $file_type != "gif") {
    ...
    $sql = 'UPDATE energie_gaz SET nom=:nom, prix_an=:prix_an,prix_kwh=:prix_kwh,prix_abonnement_annuel=:prix_abonnement_annuel,contact=:contact,name=:name ,size=:size ,data=:data ,type=:type  WHERE id=:id';
    $statement = $conn->prepare($sql);
